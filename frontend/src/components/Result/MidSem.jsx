const MidSem = ({ exams }) => {
  ...
  return (
    <div className="bg-slate-800 p-6 sm:w-[45%] w-[95%] mx-auto rounded-lg shadow-lg mb-6">
      <h3 className="text-xl font-bold text-blue-400 mb-4">Mid-Sem Marks</h3>
      <ul>
        {exams.map((exam, index) => (
          <li key={index} className="mb-4">
            {/* Course Name in green */}
            <div className="text-green-400 font-bold">{exam.course_name}</div>
            {/* Marks out of total marks, in white text */}
            <div className="text-white">
              Marks: {exam.marks} / {exam.total_marks}
            </div>
            {/* Teacher Name in white */}
      ...
      <div className="text-green-400 mb-4">
        <strong>Mid-Sem GPA:</strong> {calculateGPA(exams)}
      </div>
    </div>
  );
};
